...
// leaflet-image
module.exports = function leafletImage(map,_cfg, callback) {
    ...
    //apply order property to draw leaflet layer
    for(i in cfg.layer_order){
    	currentOrder = cfg.layer_order[i];
    	
    	//apply order for all map
    	switch(currentOrder){
		case "paths":
    	...
    	map.eachLayer(function(l){
    		switch(currentOrder){
    			...
    			case "markers":
    				...
    				break;
    			case "custom_canvas":
    		    	for(i in cfg.custom_canvas_property){
    		    		propertyName = cfg.custom_canvas_property[i];
    		  		
    		    		if(hasProperty(propertyName,l)){
    		        		layerQueue.defer(handlerCanvasLayer, getProperty(propertyName,l));   			
    		    		}
    		    	}    				
    				break;
    		}
    ...
    function handleTileLayer(layer, callback) {
        ...
        function loadTile(url, tilePos, tileSize, callback) {
        	...
        	if(        			       			
    			cfg.tiles_loader.preload_tile == true
    			&& cfg.tiles_loader.default_tile_src
        	){       		
        		//try load tile by origin url. Create img with specific url on failed
            	$.ajax({
        	        type: "GET",
        	        url: url,
        	        async: false,
        	        ...
        	        error: function (){
        	            im.src = cfg.tiles_loader.default_tile_src;
        	        }
        	    });        		
        	} else {
	            im.src = url;
    ...
    function handlerCanvasLayer(_canvas,callback){
        var canvas = document.createElement('canvas');
        canvas.width = dimensions.x;
        canvas.height = dimensions.y;
